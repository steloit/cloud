...
> **Progress log**
> **Pass 6 — done (2026-07-11, uncommitted). The audit's final pass.** Consistency polish across four workstreams. (a) **h1 grammar**: 38 files unified to "Area · Thing" — every service tab product-prefixed (PostgreSQL · SQL Editor…), Account/Assistant/Dashboards families prefixed, frame-divergence noted per file; the "Metrics" title collision is gone. (b) **Dead affordances**: O2's ⚑ genuinely opens the pre-filled U8 drawer; M3's environment buttons, the Observe snav Dashboards nit, W3's Open DLQ, project-policy creation (→ the live G7 wizard), the traces→D4 action, the db-main Restore tile, all six overview header actions and ~14 Act-zone tiles are now real links to their live tabs; ~15 fake ▾ dropdowns de-afforded with the frame text kept; T3's fake select became a real disabled one. (c) **Stale vocabulary purged**: every "Phase N" disabledReason (49 strings) either wired or rewritten to the honest finding voice naming the concrete API gap; zero remain outside comments. (d) **Shell + tokenization**: rail active-state substring bug fixed (service context wins; exactly one .rit.on), assistant button global to match ⌘J, the advertised "?" shortcut bound, snav back-chevrons are real links, menu classes deduped, org-avatar gradient extracted to one .orggrad class, warn-badge ink fixed for light mode, DP2's gates table wrapped, 5 template-literal classNames → cn(), overviews' 7 raw buttons → Btn. (e) **Scope leaks**: snav-dashboards/assistant subs, settings identity strings and the assistant drawer's context all derive from route/queries (borealis shows its own name; canon renders byte-identical); DB1's disabled-card reasons are visible inline; P6 quiet-hours got a dirty-tracked gated Save. Verified: gates green; 32/32 headless checks (single rail active on 4 page types, ? shortcut, back-link nav, 5 h1 spot-checks, U8 prefill from O2, borealis scope proof, tile/header links, no stale reasons); zero page errors.
> **Pass 5 — done (2026-07-11, uncommitted).** Missing screens & detail views. Built: O2's four category tabs as real panes on canon telemetry (Databases/Queue/Network/Cost — frame-unspecced, noted; Compute byte-identical); O5 History (the one canon firing, 14:02→14:41) + Silences (empty state, gated create); O6 trace selection (tr_8814 exemplar, honest card for the rest); D4 insight-row accordions with the canon SeqScan plan; D8 message selection (9f221 exemplar payload, honest metadata for 9f224); W5 branch accordions (derived conn-string Copybit, gated promote/delete); all six C5 gallery previews live via a verbatim-registry gallery detail; a generic completed-deployment detail (dep_142 tells the incident, gate-aborted images can't be rollback targets — "the modal must not lie"; #141 gets an honest not-in-fixtures card since the D19 frame rows diverge from fixtures — finding); DP1/D19 rows link to details; DB6 Use-layout live through the prefilled real create-modal; DB5/plan reasons honest; B5 current plan derived from the subscription. **Frame-truth corrections during review:** two Pass-4/5 agent changes had dropped frame-specced blocked states — restored verbatim: G5 "Delete Acme…" blocked (4 projects, 12 members, active subscription), G1 blocked (7 services across 3 environments), B5 downgrades blocked ("the button says why") — the typed-confirm modals remain as the unblocked-path artifacts. New findings: DP1 frame shas diverge from fixture git_shas; #141 exists in frames but not fixtures; O5 History rule-name divergence (api-p95-burn vs p95-slo). Verified: gates green; 27/27 headless checks; zero page errors.
> **Pass 4 — done (2026-07-11, uncommitted).** The overlay census. New primitives: `ConfirmModal`/`TypedConfirmModal` (`design-system/confirm.tsx` — one confirmation idiom; `gatedReason` moves the B6 grammar inside the overlay with a reason ladder) and `RenameModal` (`features/common/`); eight echo-style MSW handlers (PATCH/DELETE org·project·service·template·dashboard, DB8 precedent). Built: org/project rename + typed-confirm deletes (real endpoints), remove-member + invite-revoke confirms (two-click arms retired), template rename/delete (T2's static text now real), token-revoke confirm, change-password modal (full validation ladder), set-budget, replace-card, invoice-line inline expansion with per-line dispute affordance, snooze duration menu, dashboard-delete confirm, binding-revoke confirm, ONE rollback idiom via shared `RollbackConfirm` (the header rollback that fired with no confirm now confirms; DP1 + D19 share it), purge + FLUSHALL typed-confirms (honest endpoint-gate after the name-gate), shared `EditShapeDrawer` (postgres + valkey, wired to updateService echo), add-secret drawer (web), edit-TTL modal (D3), restore-to-branch drawer (D5, canon snapshots + PITR), service rename (postgres/valkey settings), U8 webhook inline config (judgment: no drawer-over-drawer). Skipped with reasons: redrive/discard/cancel-deploy confirms, rotate-binding, key revokes, domain/schedule renames (no endpoint AND no enabled affordance). New spec findings: PATCH /services body carries no `name` (rename is a spec gap); G1's frame says "type the project slug" but the typed-confirm contract is the name. Verified: biome/tsc/vitest/build green; 32/32 headless interaction checks (typed-confirm gates before typing and enables on exact name, honest endpoint-gate reasons surfaced after typing, Esc everywhere, one real revoke round-trip); zero page errors.
