    ...
    if (status === 410) {
      return (
        <CenterCard icon="s-cron" tone="warn">
          ...
          {renewed ? (
            <p className="text-[12.5px] text-ok">Requested — the inviter has been notified.</p>
          ) : (
            <div className="flex items-center gap-3">
              ...
              <Link to="/login" className="text-[12px] font-medium text-steel">
                Go to sign in
              </Link>
            </div>
          )}
          <div className="mono text-[10.5px] text-ink3">inv_7d21aa · expired 04 Jul 2026</div>
        </CenterCard>
      );
    }
    return (
      <CenterCard icon="s-shield">
        ...
        <div className="text-[12px] text-ink3">Ask Priya for a new link · Contact support</div>
  ...
  if (session && hint && !matchesHint(session.email, hint)) {
    return (
      <CenterCard icon="s-user" tone="err">
        ...
        <div className="text-[12px] text-ink3">Ask Priya to invite {session.email}</div>
        <div className="mono text-[10.5px] text-ink3">
    ...
    <CenterCard>
      ...
      <Card className="bg-surface2 p-3.5">
        ...
        {(data.role_consequences ?? []).map((line) => (
          <p key={line} className="text-[12px] leading-relaxed text-ink2">
            {line}
          </p>
        ))}
      </Card>
      <ul className="flex flex-col gap-1.5 text-[12px] text-ink2">
        <li>✓ Accepting gives you access immediately — no approval step follows</li>
        <li>✓ Joining is logged in the org audit trail, like every membership change</li>
      </ul>
      {session ? (
        <div className="text-[12px] text-ink3">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          type="button"
          className="text-[12px] font-medium text-ink2 hover:text-ink1"
          onClick={() =>
