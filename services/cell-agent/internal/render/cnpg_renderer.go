...
func NewCNPGRenderer(pg cnpgDriver, applier kube.Applier, cell string, log *slog.Logger) *CNPGRenderer {
...
func placementOf(svc agent.DesiredService) (placement, error) {
	p, _ := svc.Desired["placement"].(map[string]any)
	ns, _ := p["namespace"].(string)
	if ns == "" {
		return placement{}, fmt.Errorf("render: service %s has no resolved namespace in desired.placement", svc.ID)
	}
	gsa, _ := p["gsa"].(string)
	wal, _ := p["wal_bucket"].(string)
	return placement{Namespace: ns, GSAEmail: gsa, WALBucket: wal}, nil
}
...
func (r *CNPGRenderer) Converge(ctx context.Context, svc agent.DesiredService) (string, error) {
	...
	if svc.Status == "deleting" || asBool(svc.Desired["deleting"]) {
		if err := r.applier.Delete(ctx, place.Namespace, svc.ID); err != nil {
			return "", fmt.Errorf("render: delete %s: %w", svc.ID, err)
		}
		r.log.Info("converged: teardown applied", "service", svc.ID, "namespace", place.Namespace)
		return "gone", nil
	}

	spec := driver.Spec{
		Name: svc.ID, Namespace: place.Namespace, Product: svc.Product,
		Intent: asString(svc.Desired["intent"]), Shape: asMap(svc.Desired["shape"]),
		Instances: instancesOf(svc.Desired), Cell: r.cell,
		GSAEmail: place.GSAEmail, WALBucket: place.WALBucket,
	}
	...
	if err := r.applier.Apply(ctx, place.Namespace, objs); err != nil {
		return "", fmt.Errorf("render: apply %s: %w", svc.ID, err)
	}
	phase, err := r.applier.Observe(ctx, place.Namespace, svc.ID)
	if err != nil {
