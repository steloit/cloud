...
run "this_cell_enforces_network_policy" {
  ...
  assert {
    ...
    error_message = "the cell0 cell does not enable GKE Dataplane V2, so NOTHING ENFORCES NetworkPolicy: the API server would accept and store D7's default-deny policies and never drop a packet. Create-time only — changing it later rebuilds the cluster and its node pools."
  }
  ...
  assert {
    condition     = module.gke_cell.deletion_protection == true
    error_message = "the cell0 cell's deletion_protection is not true — this env's chosen setting is not reaching the cluster."
  }
}
