...
- **Silent acceptance** (E3 — `docs/plan/engineering-os-review-2026-07.md`): validate at the
  ...
  fires, one-shot resources may already be consumed. Refuse a wrong TYPE, never default it,
  or the system prices one thing and stores another. (`services.intent` carried a CHECK with
  no handler peer until US-3.7; three tests asserted on an impossible API state.)
