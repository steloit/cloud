...
## 3 · Database: PostgreSQL — everywhere, one operator *(v1.3, ADR-0003 · ADR-0007/A6: dev/alpha storage = GKE PD-CSI)*

- **Customer databases:** **CloudNativePG-operated vanilla PostgreSQL — one cluster per project-environment** (single-instance for free/dev tiers; replicas are a paid knob), on **GKE PD-CSI (`pd-balanced`) volumes** at dev/alpha (ADR-0007, measured: branch e2e 52.4 s, wake 8.0 s, restore-to-new 55.2 s; OpenEBS ZFS-LocalPV is retained as the Cell-1 branch-density option per INF-001 A6 — an explicit measured trigger, kit in `infra/spike/`). **Branching = CSI VolumeSnapshot (PD incremental, delta-priced) → CNPG recovery → hibernated single-instance cluster**; branch metadata, routing, lifecycle, and cleanup live in *our* control plane (Xata OSS is the Apache-2.0 reference implementation; pgstream where masked data-sync feeds are needed). Scale-to-zero = CNPG declarative hibernation + idle detection; wake-on-connect via the cell gateway. PITR = WAL archiving to GCS with `archive_timeout` ≤ 5 min (meets A1.3's RPO by construction); restore is always to a **new** branch. Pin CNPG ≤1.30 (in-tree Barman) — **a hard ceiling: in-tree Barman is removed in 1.31** (ADR-0007 F6); the barman-cloud plugin migration is a tracked T1.2 item. A scheduled base backup is load-bearing from day one (WAL alone is not restorable — F3); recovery targets derive from the last archived WAL position, never wall-clock (F4). D3's branching *requirement* is unchanged; the engine changed (INF-001 §A4 records why — the Neon OSS upstream went dark and self-hosting was never supported).
