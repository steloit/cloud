...
| **Urgent (security/paging) notification bypass has no SOURCE yet** (US-10.2): NotifyInput.Urgent bypasses quiet hours (tested), but no security/paging notification is emitted anywhere to set it — the first (suspicious sign-in, MFA change, etc.) arrives with the E7/auth security surface. Primitive ready, not wired. | US-10.2 | the E7/auth security-notification source sets Urgent:true when it lands |
