...
| **`testWebhook` shim is a subtree catch-all**: `MountWebhookTest` registers `POST /v1/webhooks/` (`webhooks_http.go:94`) because the contract path glues `:verb` onto a wildcard (Trap T1). Behaviourally safe today — a bad suffix 404s via `CutSuffix` + `ContainsAny(wbh,"/:")` — but it silently shadows **any** future `POST /v1/webhooks/...` sub-route registered on the strict mux, and that failure would be a silent 404 rather than a mount panic. | US-10.3 enrichment (QA review) | when `deleteWebhook`/rotate land, either restructure the path to `/webhooks/{wbh}/test` so it generates natively, or narrow the shim's pattern and add a test asserting sibling POST routes stay reachable |
