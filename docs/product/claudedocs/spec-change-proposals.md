...
| **listThreads has no org param** (T13.3): the contract carries no org selector, so listThreads unions the user's threads across all their orgs (omitting AI-disabled ones). A `?org=` filter would need an owner-level openapi change. | T13.3 | add `?org=` to listThreads if per-org scoping is wanted |
