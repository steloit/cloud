...
| **Hard cap: concurrency race (TOCTOU) at the accept/scale gate** (T11.6, HIGH): `enforceBudget` reads the run-rate then the write (InsertService/UpdateServiceShape) happens without an enclosing serializable tx or per-org lock — two concurrent same-org creates can each pass the check and both commit, transiently overshooting the cap (bounded: the next op sees the higher committed and refuses). For a flagship "impossible by construction," this window should be closed. | T11.6 review (arch+QA) | wrap the cap-check + the committing write in one tx holding `pg_advisory_xact_lock(hashtext(org_id))` (store has `WithTx`); the estimate `Accept` stays a pre-check burn-guard, the locked check is authoritative — its own task (touches the provisioning write path) |
