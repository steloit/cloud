...
2. **Cardinality:** **fleet** (pointable instances — Postgres, Valkey, Web, Worker) or **capability** (one endpoint, config-shaped — *no current service has this shape post-A5*; the shape remains available to future services)? This decides: count badge and instance switcher (fleet, rendered only at n>1) vs. neither, ever (capability). See spec §Extensibility.
