...
**Positioning note (2026-08-22 · `docs/plan/positioning-v2.md`):** positioning v2 re-aims what this plan is *for* — the operating gap (applications get written far faster than they get run), not certainty-as-differentiator. **No sequencing in this plan changes**: the D11 alpha path already begins at the repository and ends at a running service, which is exactly the v2 loop. What changes is emphasis — application understanding and infrastructure recommendation are the headline of the alpha demo, and the branched preview is its closing capability rather than its point. Milestones, epics, estimates, and exits below are unchanged.
...
3. **Five committed clients are waiting.** The MVP is the alpha path they signed up for — *connect repo → cost shown → approve → deployed service + Postgres → open PR → branched preview* (D11). It is the v2 loop end to end: the repository is the input, a running application is the output, and the developer configures nothing in between. Everything is sequenced to put that in their hands fastest, then expand to the full platform behind them. (Narrative order is `docs/plan/positioning-v2.md`'s, not this plan's — see `17-brand/messaging.md`.)
