...
| — **15th API: `cloudresourcemanager.googleapis.com`** | ❓ **NEEDS FOUNDER INPUT** — see `tasks/e1-substrate/T1.7.md`. The list above and `infra/modules/project-base/main.tf` agree on the same 14, and **both omit it** — so this is a gap in the authoritative configuration, not implementation drift. `gke-cell`'s `google_project_iam_member.node_roles` (the least-privilege node SA's five roles) is served by that API: measured on the wire, `projects:getIamPolicy` resolves to `https://cloudresourcemanager.googleapis.com/v1/...`, so a from-zero `envs/dev` cannot grant them. **Disclosure: `steloit-dev` already has it enabled** — enabled by hand during US-3.3c so the live cell could be built, which makes the running project a third representation that currently disagrees with both declared ones. Recommended: add it to the list and to the bootstrap procedure's API count. The API is free and read/write only on project IAM, which Terraform already exercises. Implementation must not add it silently (founder instruction, 2026-08-25). | ❓ |
...
| **What a SIZE downgrade costs while storage is retained** | — | ❓ **NEEDS FOUNDER INPUT.** Today `standard`(50 GB) → `dev` prices `1900 + 50×50¢ = 4400¢`: the catalog's own formula on the resulting configuration (ADR-041-compliant), but a **2.3× rise on an action the customer calls a downgrade**, with no consent step and no cost breakdown on the wire. Market convention differs — Cloud SQL refuses a storage decrease outright; RDS/Aurora, Neon, Supabase and PlanetScale price storage as a separate dimension so a compute downgrade never inherits a storage bill. Three options: **(a)** keep the arithmetic and surface a breakdown + consent before the PATCH commits; **(b)** refuse the size downgrade while storage exceeds the new size's included amount; **(c)** decouple storage from the size's base price entirely. An earlier revision of this file asserted (a) as settled on the implementer's authority — it is not ruled, and this section requires an ADR. |
