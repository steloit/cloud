...
| **Postgres `included_gb` semantics** | storage the customer **GETS**, not merely "not charged extra". An unset — or lower — `storage_gb` resolves UP to the size's `included_gb` (**50 GB** for `standard` and `performance`, 0 for `dev`), so 0/30/50 on a standard are one contract. The price is unchanged: only `storage_gb - included_gb` above zero is billed. `performance`'s 50 is analogy with `standard`, not derived — canon prices no performance service (`pricing.json` `$note`, S9 finding). | **founder 2026-08-23** · T3.4c · `estimates/pricing.json` |
| **Storage is a RATCHET** | a service's `storage_gb` never decreases. A PersistentVolumeClaim cannot shrink — Kubernetes supports expansion only and rejects a request below `.status.capacity` — so a size **downgrade keeps the volume and bills it**: `standard`(50 GB) → `dev` prices `1900 + 50×50¢ = 4400¢`, because dev includes 0. This is the catalog's own arithmetic on the resulting configuration, not a separate rule; billing dev's base alone would give away storage the cluster still carries. Rows created before T3.4c are reconciled by migration `20260823120000`, which is **price-neutral at rest**. | T3.4c · [k8s.io/docs/concepts/storage/persistent-volumes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/) |
