...
| **HA is priced as a flat uplift, and every comparable is a multiplier** | ❓ **NEEDS FOUNDER INPUT** — see `tasks/e3-provisioning/US-3.19.md`. `ha_cents: 1900` is flat, so the uplift is **100% on `dev`, 32.8% on `standard`, 17.0% on `performance`**. Every published comparable is 2x-3x: Cloud SQL "an HA-configured instance costs twice as much… includes CPU, RAM, and storage", Crunchy "doubles the cluster price", PlanetScale's 3-node HA is ~3x. **This became a live margin leak on 2026-08-25**, when US-3.16 made `ha: true` actually provision a second instance — before that it rendered one instance and cost us nothing, so the mis-pricing was invisible. A `performance` + `ha` customer now costs a full second cluster and pays $19 for it. Under ADR-0018's allocated-resource pricing this **disappears**: two instances meter twice and `ha_cents` is deleted rather than re-rated. If that direction is taken, rule the rates and this row closes with it. If not, `ha_cents` needs a multiplier. Implementation must not pick either. | ❓ |
