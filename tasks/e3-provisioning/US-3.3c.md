...
7. **`endPort` must be REFUSED, not written.** Dataplane V2 silently does not
...
8. **Customer code blocked from `169.254.169.254`; managed CNPG allowed.** GKE
...
9. **`infra/k8s/policy/network-logging.yaml` is applied by something**, and a
   ...
   nothing installs.
10. **NetworkLogging's coverage of gVisor pods is confirmed either way.** Google
